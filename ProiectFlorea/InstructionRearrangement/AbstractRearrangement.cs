    ...
    public class AbstractRearrangement
    {
        ...
        public List<string> Rearrange()
        {
            ...
            for (var index = 0; index < megaInstructions.Count - 1; index++)
            {
                ...
                if (InstructionRearrangementUtil.CanApplyImmediateMerging(megaInstructionOne, megaInstructionTwo))
                {
                    ApplyImmediateMerging(megaInstructionOne, megaInstructionTwo);
                    handledLines.Add(megaInstructionOne.ToString());
                }

                if (InstructionRearrangementUtil.CanApplyMovMerging(megaInstructionOne, megaInstructionTwo))
                {
                ...
                if (InstructionRearrangementUtil.CanApplyMovReabsorption(megaInstructionOne, megaInstructionTwo))
                {
                    ApplyMovReabsorption(megaInstructionOne, megaInstructionTwo);
                    handledLines.Add(megaInstructionOne.ToString());
                }

                if (InstructionRearrangementUtil.CanApplyMemoryAntiAlias(megaInstructionOne, megaInstructionTwo))
                {
